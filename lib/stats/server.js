/**
 * Vendored from dsh-usage-stats (MIT)
...
import { applyCallDelta, applyUsageDelta, createCallState, createUsageState, mergeInto, renderUsage, totalTokens, zeroBuckets } from "./usage.js";
...
const CALL_MAX = 100000;
const callStore = { sessions: {} };
let callInflight = null;

function flattenCallRecords() {
	const out = [];
	for (const state of Object.values(callStore.sessions)) {
		if (state?.byKey instanceof Map) for (const rec of state.byKey.values()) out.push(rec);
	}
	out.sort((a, b) => a.time - b.time);
	if (out.length > CALL_MAX) out.splice(0, out.length - CALL_MAX);
	return out;
}

function resetCallState(state) {
	state.byKey = new Map();
	state.currentModel = null;
	state.consumed = 0;
}
...
/**
 * Reconstruct per-call records from the same live + persisted session logs
 * that feed `/api/usage-stats/usage`, so 概览 / 用量 / 调用记录 match the
 * 14-day token bars.
 */
export async function collectCallRecords(ctx) {
	if (callInflight !== null) return callInflight;
	callInflight = collectCallRecordsNow(ctx).finally(() => {
		callInflight = null;
	});
	return callInflight;
}

async function collectCallRecordsNow(ctx) {
	const live = ctx.get("sessions");
	const attached = new Set();
	if (live !== void 0) {
		for (const session of live.list()) {
			attached.add(session.id);
			const state = callStore.sessions[session.id] ?? createCallState();
			if (state.kind !== "live") resetCallState(state);
			const count = session.events.length;
			if ((state.consumed ?? 0) < count) {
				applyCallDelta(state, session.events.slice(state.consumed ?? 0));
				state.consumed = count;
			}
			state.kind = "live";
			callStore.sessions[session.id] = state;
		}
	}
	const persistence = ctx.get("sessionPersistence");
	const persistedIds = new Set();
	if (persistence !== void 0) {
		...
		if (typeof persistence.listSnapshots === "function") {
			try {
				...
				ctx.logger?.warn?.(`usage-stats: listSnapshots failed (calls): ${String(error)}`);
			}
		}
		const metas = snapshots !== null ? snapshots.map((entry) => entry.header) : await persistence.list();
		const revisionOf = new Map();
		...
		for (const meta of metas) {
			persistedIds.add(meta.id);
			if (attached.has(meta.id)) continue;
			const state = callStore.sessions[meta.id] ?? createCallState();
			const revision = revisionOf.get(meta.id);
			const changed = state.kind !== "persisted" || (revision !== void 0 && revision !== state.revision) || revision === void 0;
			if (changed) {
				try {
					...
					const fromSeq = wasPersisted ? state.consumed : 0;
					const { events } = await persistence.readFrom(meta.id, fromSeq);
					if (!wasPersisted) resetCallState(state);
					const fresh = wasPersisted ? events.filter((event) => event.seq > (state.consumed ?? 0)) : events;
					const contiguous = fresh.length === 0 ? state.consumed === 0 : fresh[0].seq === state.consumed + 1;
					if (!contiguous && state.consumed > 0) {
						resetCallState(state);
						const { events: allEvents } = await persistence.readFrom(meta.id, 0);
						applyCallDelta(state, allEvents);
						state.consumed = allEvents.length > 0 ? allEvents[allEvents.length - 1].seq : 0;
					} else if (fresh.length > 0) {
						applyCallDelta(state, fresh);
						state.consumed = fresh[fresh.length - 1].seq;
					}
					state.kind = "persisted";
					if (revision !== void 0) state.revision = revision;
				} catch (error) {
					ctx.logger?.warn?.(`usage-stats: reading persisted session "${meta.id}" for calls failed: ${String(error)}`);
				}
			}
			callStore.sessions[meta.id] = state;
		}
	}
	for (const id of Object.keys(callStore.sessions)) {
		if (!attached.has(id) && !persistedIds.has(id)) delete callStore.sessions[id];
	}
	return flattenCallRecords();
}
