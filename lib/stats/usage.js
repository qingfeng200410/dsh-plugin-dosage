/**
 * Vendored from dsh-usage-stats (MIT) — https://github.com/Ychris12138/dsh-usage-stats
...
/** Per-session fold state for reconstructing individual call records. */
...
function splitModelKey(model) {
	...
	if (slash <= 0) return { provider: "", model: String(model || "") };
	return {
		provider: model.slice(0, slash) === "unknown" ? "" : model.slice(0, slash),
		model: model.slice(slash + 1)
	};
}

/**
 * Fold NEW events onto a per-call map. Same (turn, step) replaces the
 * earlier sample, matching `applyUsageDelta` so day totals and the call
 * list stay aligned.
 */
export function applyCallDelta(state, events) {
	...
	for (const event of events) {
		...
		const modelKey = modelOf(event) ?? currentModel ?? "unknown/unknown";
		const parts = splitModelKey(modelKey);
		const usage = sample.usage ?? {};
		state.byKey.set(sample.key, {
			time: event.time,
			model: parts.model,
