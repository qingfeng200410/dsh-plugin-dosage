  ...
  factory: (require) => {
    ...
    function UsagePanel(props) {
      ...
      React.useEffect(function () {
        refresh();
        // 每 5 秒自动刷新（列表已分页渲染，避免 3 秒全量重取 + 全量重渲染导致的卡顿）
        if (timer && timer.interval) return timer.interval(refresh, 5000);
        return undefined;
      }, []);
    ...
    // ── Sidebar usage/balance panel (vendored from dsh-usage-stats, MIT) ──
        ...
        /** Prefer the host timer (pauses in background); otherwise `setInterval`. */
        function bindInterval(timer, fn, ms) {
        ...
        function UsageStatsPanel({ wide, t, timer }) {
            ...
            // 侧栏「用量/余额」数字来自今日 Token：浮层关着也每 5 秒拉一次，不必点开。
            react.useEffect(() => {
                loadUsage();
                return bindInterval(timer, loadUsage, 5000);
            }, [loadUsage, timer]);
            ...
            react.useEffect(() => {
                ...
                const billingTimer = window.setInterval(loadBilling, 5000);
                return () => {
                    window.clearInterval(providerTimer);
                    window.clearInterval(billingTimer);
                };
            }, [open, loadProviders, loadBilling]);

            // 面板打开时每 5 秒强制拉一次上游余额，不走五分钟缓存。
            react.useEffect(() => {
                if (!open || selectedProvider === null) return;
                loadAccount(selectedProvider, true);
                const accountTimer = window.setInterval(() => loadAccount(selectedProvider, true, true), 5000);
                return () => {
